...
class STLS1(object):
    ...
    def create_garp_401 (self):
        t_isg = self.time_sec * 1000000.0
        base_garp_pkt =  Ether(src=self.src_mac, dst="ff:ff:ff:ff:ff:ff")/Dot1Q(vlan=41)/ARP(psrc="10.41.0.1", hwsrc=self.src_mac, hwdst=self.src_mac, pdst="10.41.0.1")
        pkt = STLPktBuilder(pkt=base_garp_pkt)

        return STLStream( name = 'LOAD_GARP_401',
                          packet = pkt,
                          mode = STLTXSingleBurst( pps = 3, total_pkts = 3),
                          next = 'NULL_401')
                         
    def create_garp_dummy_401(self):
        t_isg = self.time_sec * 1000000.0
    ...
    def get_streams (self, direction = 0, **kwargs):
        ...
        return [
          self.create_garp_401(),
          self.create_garp_dummy_401(),
          ]
